...
namespace IotDashboard.Api.Controllers
{
    ...
    public class TelemetryController : ControllerBase
    {
        ...
        [HttpGet("site/{siteId}/latest")]
        public async Task<IActionResult> GetLatestBySite(string siteId, CancellationToken cancellationToken)
        {
            var res = await _telemetryHandler.GetLatestBySiteAsync(siteId, cancellationToken);
        ...
        [HttpGet("site/{siteId}/history")]
        public async Task<IActionResult> GetHistoryBySite(
            string siteId,
            [FromQuery] DateTime? fromUtc,
            [FromQuery] DateTime? toUtc,
            [FromQuery] int limit = 100,
            CancellationToken cancellationToken = default)
        {
            var res = await _telemetryHandler.GetHistoryBySiteAsync(
                siteId,
                fromUtc,
                toUtc,
                limit,
